...
- 本地代码通过 `CLOBVersion` 显式区分 V1/V2 订单结构，避免 host 与签名 schema 不匹配
- 官方 SDK 已切换到新包名：
...
- 对一个直接在 Polymarket.com 使用的真实账户，`signatureType=0 (EOA)` 查询 `GET /balance-allowance` 时返回 `balance="0"`
- 将 `signatureType` 改为 `2 (GNOSIS_SAFE)` 后，同一账户可查到正确余额
- 这说明很多 Polymarket.com 账户虽然由用户自己的私钥发起签名，但资金上下文仍落在 proxy wallet / safe，而不是裸 EOA
- 因而“余额查询成功所用的 `signatureType`”应视为下单时的优先候选值；不要默认把这类账户按 `0=EOA` 处理
...
- V1 与 V2 需要分支处理，不能把某一个版本的 order struct 混用于所有 host
- V1 wire/signature 保留 `taker`、`nonce`、`feeRateBps`
- V2 wire/signature 使用 `timestamp`、`metadata`、`builder`
- host 与 `CLOBVersion` 不匹配时，常见表现是旧端要求 `feeRateBps` 或新端返回 `invalid signature`
...
没有直接解释 `owner` 应如何获取。官方 Python SDK 的下单序列化逻辑会把 `owner` 设置为当前 API key。

结论：

- Go 客户端在持有 L2 凭证时，可以默认把 `owner` 设为 API key
- 调用方仍可显式覆盖 `owner`，以兼容未来服务端或 SDK 行为变化
...
### 8.4 `cancel-market-orders` 过滤参数描述不一致

高层交易文档和 API Reference 对这个端点的请求体要求并不完全一致：

- 一个版本把 `market`、`asset_id` 描述为可选过滤条件
- 另一个版本把这两个字段标成 required

本地实现建议：

- 不允许空请求直接打 `DELETE /cancel-market-orders`
- 至少要求提供 `market` 或 `asset_id` 中的一个
- 如果要“全撤”，优先走专门的 `DELETE /cancel-all`

这样可以避免因为文档歧义而把“按市场撤单”误用成“全撤”。

### 8.5 `heartbeats` 的请求体在文档与 SDK 中存在差异

API Reference 页面把 `POST /heartbeats` 展示为一个无 body 的请求。

但官方交易文档和官方 TypeScript SDK 又都表明：

- heartbeat 会话和 `heartbeat_id` 有关
- SDK 的 `postHeartbeat` 会在提供 heartbeat id 时发送 JSON body

本地实现建议：

- `SendHeartbeat()` 支持在提供 heartbeat id 时发送 `{"heartbeat_id":"..."}` 并参与 L2 签名
- 当调用方未提供 heartbeat id 时，允许先发一个无 body 请求，后续再根据服务端行为补齐会话逻辑
